...
# --------------------------------------------------------------------------- #
# Dependency management (portable — no system packages)
# --------------------------------------------------------------------------- #
...
def _ensure_ffmpeg(auto_yes=False):
    ...
    url = FFMPEG_RELEASE_URLS.get("linux")
...
def _ensure_rife(auto_yes=False):
    ...
    url = RIFE_RELEASE_URLS.get("linux")
    ...
    for vendor, name in pci_gpus:
        if vendor == "nvidia":
            ...
            if not any(k in vk_names for k in ("amd", "radeon")):
                ...
                status(_("Install the Vulkan driver for your AMD GPU (e.g., vulkan-radeon)."), "WARN")
        elif vendor == "intel":
            if not any(k in vk_names for k in ("intel", "uhd", "iris")):
                status(f"{_('Intel GPU detected')} ({name}) {_('without active Vulkan driver.')}", "WARN")
                status(_("Install the Vulkan driver for your Intel GPU (e.g., vulkan-intel)."), "WARN")
...
def ensure_dependencies(auto_yes=False):
    ...
    if shutil.which("vulkaninfo") is None:
        status(_("vulkaninfo not found (optional, GPU detection limited)."), "WARN")
...
def probe_video_file(path):
    ...
    audio_cmd = [
        str(FFPROBE_BIN), "-v", "error", "-select_streams", "a",
