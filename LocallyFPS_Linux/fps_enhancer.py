...
LocallyFPS – frame interpolation tool using RIFE AI models.
"""
...
# --------------------------------------------------------------------------- #
# Interactive selector (arrow keys + enter)
# --------------------------------------------------------------------------- #
...
def list_available_rife_models():
    if not RIFE_INSTALL_DIR.is_dir():
        return ["rife-v4.6"]
    found = sorted(d.name for d in RIFE_INSTALL_DIR.iterdir()
                    if d.is_dir() and d.name.startswith("rife-"))
    return found if found else ["rife-v4.6"]

def _apply_preset(label):
    vals = PRESETS[label]
    CONFIG["encoder"] = vals["encoder"]
    CONFIG["preset"] = vals["ffmpeg_preset"]
    CONFIG["crf"] = vals["crf"]
    CONFIG["model"] = vals["model"]
    CONFIG["video_preset"] = label
        ...
        if i == 0:
            ...
            if sub == 0:
                ...
                if 0 <= mi < len(available):
                    ...
                    if not (RIFE_INSTALL_DIR / selected).is_dir():
...
RIFE_INSTALL_DIR = Path("/opt/rife-ncnn-vulkan")
...
PACKAGE_MANAGERS = {
    "pacman": {
        ...
        "packages": {
            ...
            "mesa-vulkan-amd": "vulkan-radeon",
            "mesa-vulkan-intel": "vulkan-intel",
            "pciutils": "pciutils",
        },
    },
    "apt-get": {
        ...
        "packages": {
            "ffmpeg": "ffmpeg",
            "vulkan-tools": "vulkan-tools",
            "mesa-vulkan-amd": "mesa-vulkan-drivers",
            "mesa-vulkan-intel": "mesa-vulkan-drivers",
            "pciutils": "pciutils",
        },
    },
    "dnf": {
    ...
    "zypper": {
        ...
        "packages": {
            "ffmpeg": "ffmpeg",
            "vulkan-tools": "vulkan-tools",
            "mesa-vulkan-amd": "Mesa-libVulkan_radeon",
            "mesa-vulkan-intel": "Mesa-libVulkan_intel",
            "pciutils": "pciutils",
        },
    },
    "apk": {
        ...
        "packages": {
            "ffmpeg": "ffmpeg",
            "vulkan-tools": "vulkan-tools",
            "mesa-vulkan-amd": "mesa-vulkan-ati",
...
def install_package(pm_name, logical_pkg):
    info = PACKAGE_MANAGERS[pm_name]
    real_pkg = info["packages"].get(logical_pkg)
    if not real_pkg:
        return False
    cmd = info["install"] + [real_pkg]
    status(f"{_('Running:')} {Color.dim(' '.join(cmd))}")
...
def _offer_install(pm, logical_pkg, human_name, required=True):
    if pm is None:
        ...
        if required:
            sys.exit(1)
        return False
    if ask_yes_no(f"{_('Install')} {Color.bold(human_name)} {_('now with')} {pm}{_('? (may ask for sudo password)')}", default=True):
...
def install_rife_ncnn_vulkan():
    ...
    if not ask_yes_no(_("Download and install it now? (~400 MB, requires sudo)"), default=True):
        status(_("rife-ncnn-vulkan is required to continue."), "ERROR")
        sys.exit(1)

    with tempfile.TemporaryDirectory() as tmpdir:
        zip_path = Path(tmpdir) / "rife.zip"
        status(_("Downloading rife-ncnn-vulkan (official release)..."))
        ...
        status(f"{_('Installing to')} {RIFE_INSTALL_DIR} {_('(needs sudo)...')}")
        try:
            subprocess.run(["sudo", "mkdir", "-p", str(RIFE_INSTALL_DIR)], check=True)
            subprocess.run(["sudo", "cp", "-r", f"{source_dir}/.", str(RIFE_INSTALL_DIR)], check=True)
            subprocess.run(["sudo", "chmod", "+x", str(RIFE_INSTALL_DIR / "rife-ncnn-vulkan")], check=True)
            subprocess.run(["sudo", "ln", "-sf", str(RIFE_INSTALL_DIR / "rife-ncnn-vulkan"), str(RIFE_SYMLINK)], check=True)
...
def install_model(model_name):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if not model_src.is_dir():
            ...
            return False
        status(f"{_('Installing')} {model_name} {_('(needs sudo)...')}")
        try:
            subprocess.run(["sudo", "mkdir", "-p", str(RIFE_INSTALL_DIR)], check=True)
            subprocess.run(["sudo", "cp", "-r", str(model_src), str(RIFE_INSTALL_DIR)], check=True)
        except subprocess.CalledProcessError as exc:
...
def _ensure_vulkan_driver(pm):
    pci_gpus = detect_pci_gpus()
    if not pci_gpus:
        status(_("Could not list system GPUs (missing lspci). Skipping."), "WARN")
        return
    vk_devices = detect_vulkan_gpus()
    vk_names = " ".join(d[1].lower() for d in vk_devices)
    for vendor, name in pci_gpus:
        if vendor == "nvidia":
            if not any(k in vk_names for k in ("nvidia", "geforce", "quadro", "rtx", "gtx")):
                status(f"{_('NVIDIA GPU detected')} ({name}) {_('but Vulkan does not see it.')}", "WARN")
                status(_("Make sure the proprietary NVIDIA driver is installed."), "WARN")
        elif vendor == "amd":
            if not any(k in vk_names for k in ("amd", "radeon")):
                status(f"{_('AMD GPU detected')} ({name}) {_('without active Vulkan driver.')}", "WARN")
                _offer_install(pm, "mesa-vulkan-amd", _("the Vulkan driver for AMD (mesa)"), required=False)
        elif vendor == "intel":
            if not any(k in vk_names for k in ("intel", "uhd", "iris")):
                status(f"{_('Intel GPU detected')} ({name}) {_('without active Vulkan driver.')}", "WARN")
                _offer_install(pm, "mesa-vulkan-intel", _("the Vulkan driver for Intel (mesa)"), required=False)
...
def ensure_dependencies():
    ...
    if shutil.which("vulkaninfo") is None:
        _offer_install(pm, "vulkan-tools", "vulkan-tools", required=False)
    _ensure_vulkan_driver(pm)
...
def _detect_gpu_vendors():
    ...
    for _, name, _ in detect_vulkan_gpus():
        ...
        if "amd" in n or "radeon" in n:
            vendors.add("amd")
    for vendor, _ in detect_pci_gpus():
        vendors.add(vendor)
    return vendors
...
def reassemble_video(
    ...
    cmd_prog = ["ffmpeg", "-y", "-progress", "pipe:1"] + cmd[2:]
...
def _is_tmpfs(path):
    """Check if a path is mounted as tmpfs (in RAM)."""
    try:
        ...
        with open("/proc/mounts") as f:
            for line in f:
                ...
                if len(parts) >= 3:
                    ...
                    fs_type = parts[2]
                    if fs_type == "tmpfs" and path == mount_point or mount_point in path.parents:
                        return True
    except OSError:
        pass
    return False
...
def _pick_temp_root(use_tmpfs):
    if use_tmpfs:
        shm = Path("/dev/shm")
        if shm.is_dir() and os.access(str(shm), os.W_OK):
            try:
                return Path(tempfile.mkdtemp(prefix="fps_enhancer_", dir=str(shm)))
            except OSError:
                pass
    
    tmp_path = Path("/tmp")
    if tmp_path.is_dir() and os.access(str(tmp_path), os.W_OK) and not _is_tmpfs(tmp_path):
        try:
            return Path(tempfile.mkdtemp(prefix="fps_enhancer_", dir=str(tmp_path)))
        except OSError:
            pass
    ...
    try:
        return Path(tempfile.mkdtemp(prefix="fps_enhancer_", dir=str(work_dir)))
    except OSError:
        pass
    
    return Path(tempfile.mkdtemp(prefix="fps_enhancer_"))


class TempManager:
    def __init__(self, estimated_bytes=0):
        tmp_path = Path("/tmp")
        tmp_is_tmpfs = _is_tmpfs(tmp_path)
        
        use_tmpfs = False
        if estimated_bytes > 0:
            avail_ram = _get_avail_ram()
            needed = int(estimated_bytes * 2.5)
            if avail_ram > needed and tmp_is_tmpfs and estimated_bytes < 1024*1024*1024:
                use_tmpfs = True
        
        self.temp_root = _pick_temp_root(use_tmpfs)
        ...
        if use_tmpfs:
            status(f"{_('→ frames in RAM (tmpfs).')}", "INFO")
        else:
...
def resolve_output_path(raw, input_path, target_fps):
    ...
    enhanced_dir = Path("videos/enhanced")
...
def prompt_for_video():
    videos_dir = Path("videos/original")
...
def confirm_summary(info, target_fps, output_path, gpu_settings):
    frame_count = info["frame_count"] or int(info["fps"] * info["duration"])
    est_seconds = estimate_duration(frame_count, info["width"], info["height"], gpu_settings["class"])

    print(f"\n{Color.bold(_('=== Summary ==='))}")
    print(f"  {Color.info(_('Video'))}            : {Color.bold(str(info['path']))}")
    res = f"{info['width']}x{info['height']}"
    print(f"  {Color.info(_('Resolution'))}       : {Color.bold(res)}")
    print(f"  {Color.info(_('FPS'))}              : {format_fps(info['fps'])} > {Color.bold(format_fps(target_fps))}")
    print(f"  {Color.info(_('Estimated time'))}  : ~{format_duration(est_seconds)}")
    print(f"  {Color.info(_('Will be saved to'))}   : {output_path}")
    print()
    return ask_yes_no("Start processing?", default=True)
