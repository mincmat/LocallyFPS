...
Puerto a Windows del proyecto original para Linux. Cambios principales:
  - Instala ffmpeg y rife-ncnn-vulkan en %LOCALAPPDATA%\\LocallyFPS (sin admin/sudo).
  - Detecta GPU vía WMI/PowerShell en lugar de lspci.
  - Usa hwaccel/encoders de Windows (nvenc, qsv, amf) en lugar de VAAPI.
  - Selector interactivo con msvcrt en lugar de termios/tty.
  - Sin dependencia de /proc, /dev/shm ni gestores de paquetes de Linux.
"""
...
APP_NAME = "LocallyFPS"
APPDATA_DIR = Path(os.environ.get("LOCALAPPDATA") or (Path.home() / "AppData" / "Local")) / APP_NAME

RIFE_RELEASE_URL = (
    "https://github.com/nihui/rife-ncnn-vulkan/releases/download/"
    "20221029/rife-ncnn-vulkan-20221029-windows.zip"
)
RIFE_INSTALL_DIR = APPDATA_DIR / "rife-ncnn-vulkan"

# Build estático de ffmpeg para Windows (incluye ffmpeg.exe y ffprobe.exe).
FFMPEG_RELEASE_URL = (
    "https://github.com/BtbN/FFmpeg-Builds/releases/download/latest/"
    "ffmpeg-master-latest-win64-gpl.zip"
)
FFMPEG_INSTALL_DIR = APPDATA_DIR / "ffmpeg"

# Rutas resueltas en tiempo de ejecución por ensure_dependencies()/find_*()
FFMPEG_BIN = None
FFPROBE_BIN = None
RIFE_BIN = None
...
def _find_exe_in_dir(root, exe_name):
    if not root.is_dir():
        return None
...
def find_ffmpeg():
    ...
    if p:
        return p
    return _find_exe_in_dir(FFMPEG_INSTALL_DIR, "ffmpeg.exe")
...
def find_ffprobe():
    ...
    if p:
        return p
    return _find_exe_in_dir(FFMPEG_INSTALL_DIR, "ffprobe.exe")


def find_rife():
    p = shutil.which("rife-ncnn-vulkan.exe") or shutil.which("rife-ncnn-vulkan")
    if p:
        return p
    exe = RIFE_INSTALL_DIR / "rife-ncnn-vulkan.exe"
...
def install_ffmpeg_windows():
    status(_("ffmpeg is not installed."), "WARN")
    if not ask_yes_no(_("Download and install ffmpeg now? (~120 MB)"), default=True):
        status(f"ffmpeg {_('is required to continue.')}", "ERROR")
        sys.exit(1)

    if _winget_available():
        status(_("Installing ffmpeg with winget..."))
        result = subprocess.run(
            ["winget", "install", "--id=Gyan.FFmpeg", "-e", "--silent",
             "--accept-package-agreements", "--accept-source-agreements"]
        )
        if result.returncode == 0 and (find_ffmpeg() and find_ffprobe()):
            status(_("ffmpeg installed successfully via winget."), "OK")
            return True
        status(_("winget could not install ffmpeg. Falling back to manual download."), "WARN")

    with tempfile.TemporaryDirectory() as tmpdir:
        zip_path = Path(tmpdir) / "ffmpeg.zip"
        status(_("Downloading ffmpeg (static build)..."))
        dl = DownloadProgress(_("Downloading"))
        try:
            urllib.request.urlretrieve(FFMPEG_RELEASE_URL, zip_path, reporthook=dl)
        except Exception as exc:
            status(f"{_('Download failed:')} {exc}", "ERROR")
            status(
                _("Please install ffmpeg manually from https://ffmpeg.org/download.html "
                  "and make sure ffmpeg.exe/ffprobe.exe are in PATH."), "ERROR"
            )
            sys.exit(1)
        finally:
            dl.close()
    ...
    if not (find_ffmpeg() and find_ffprobe()):
        status(_("Could not install ffmpeg automatically."), "ERROR")
        status(
            _("Please install ffmpeg manually from https://ffmpeg.org/download.html "
              "and make sure ffmpeg.exe/ffprobe.exe are in PATH."), "ERROR"
        )
    ...
    status(_("ffmpeg installed successfully."), "OK")
    return True
...
def _check_vulkan_runtime():
    """En Windows el runtime de Vulkan lo instala el propio driver de GPU.
    No hay paquete separado que instalar; solo se informa si vulkaninfo no está."""
    if shutil.which("vulkaninfo") is None:
        status(
...
class TempManager:
    def __init__(self, estimated_bytes=0):
        ...
        if estimated_bytes > 0:
            _check_disk_space(base, int(estimated_bytes * 2.5))

        self.temp_root = Path(tempfile.mkdtemp(prefix="locallyfps_"))
